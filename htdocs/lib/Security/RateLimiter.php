...
final class RateLimiter
{
    public static function hit(string $scope, string $identifier, int $limit, int $decaySeconds): RateLimitResult
    {
        ...
        if ($driver === 'mysql') {
            $sql = 'INSERT INTO rate_limits (scope, identifier, count, expires_at, last_seen)
                    VALUES (:scope, :identifier, 1, :expires_at, :now)
                    ON DUPLICATE KEY UPDATE
                        count = IF(expires_at < VALUES(expires_at), 1, count + 1),
